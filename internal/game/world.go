...
func (w *WorldState) SetCells(cells [][]Life) {
	w.cells = cells
}
...
func NewWorld(width, height int) *WorldState {
	w := &WorldState{}
	w.cells = make([][]Life, height)
	for i := range w.cells {
		w.cells[i] = make([]Life, width)
...
// populate the world with a glider
